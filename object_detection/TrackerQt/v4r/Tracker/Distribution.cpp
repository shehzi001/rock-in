...
Particle
Distribution::calcMean (const ParticleList &pl)
{
  if (pl.empty ())
  {
...
Particle
Distribution::calcWeightedMean (ParticleList &pl)
{
  if (pl.empty ())
  {
    printf ("[Distribution::calcWeightedMean] Warning ParticleList empty.\n");
...
float
Distribution::calcMedianC (const ParticleList &pl)
{
  float median;
  unsigned n = pl.size ();
...
double
Distribution::getPositionVariance ()
{
  double var = 0.0;
...
double
Distribution::getOrientationVariance ()
{
  double var_ax = 0.0;
  double var_ang = 0.0;
...
void
Distribution::normalizeWeights (ParticleList &pl)
{
  float sum = 0.0;
  float dsum;
...
void
Distribution::drawParticlesEdges (TrackerModel& model, TomGine::tgShader* shadeCompare, bool showparticles)
{
  model.setTexture (0);
...
void
Distribution::drawParticlesTextured (TrackerModel& model, TomGine::tgShader* shader, bool showparticles)
{
  m_particlelist.size ();
...
float
Distribution::confidenceFunction (const float &m, const float &e, const float& e_max)
{
  float c = 0.0f;
...
void
Distribution::calcEdgeConfidence (int convergence)
{
  int v, d;
  int v_max = 0;
...
void
Distribution::calcColorConfidence (int convergence)
{
  int v, d;
  int v_max = 0;
...
void
Distribution::updateLikelihood (TrackerModel& model, TomGine::tgShader* shEdge, TomGine::tgShader* shColor,
                                Method method, bool textured, bool fulltextured, int convergence, bool showparticles)
{
...
//    m_particlelist[i].w = m_particlelist[i].c;
    m_particlelist[i].w = pow (m_particlelist[i].c, float (convergence) * (1.0f - m_particlelist[i].c));
  ...
  // calculate Neff for lost detection
  n_eff = 0.0;
  for (size_t i = 0; i < m_particlelist.size(); i++)
    n_eff += pow(m_particlelist[i].w, 2);
  n_eff = 1.0 / n_eff;
}
...
void
Distribution::drawCoordinates (float linelength, float linewidth)
{
...
bool
Distribution::sortfunction (Particle p1, Particle p2)
{
  return (p1.w > p2.w);
}
