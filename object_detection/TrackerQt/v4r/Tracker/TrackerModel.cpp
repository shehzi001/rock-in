...
#endif
  if (glIsList (m_dlTexturedFaces))
    glDeleteLists (m_dlTexturedFaces, 1);
  if (glIsList (m_dlUntexturedFaces))
    glDeleteLists (m_dlUntexturedFaces, 1);
  if (glIsList (m_dlPass))
    glDeleteLists (m_dlPass, 1);
  if (glIsList (m_dlFaces))
    glDeleteLists (m_dlFaces, 1);
  if (glIsList (m_dlEdges))
    glDeleteLists (m_dlEdges, 1);
  if (glIsList (m_dlNormals))
    glDeleteLists (m_dlNormals, 1);
#ifdef DEBUG
...
void
TrackerModel::CopyTo(TomGine::tgTextureModel &m)
{
  m = TomGine::tgTextureModel(*this);

}

void
TrackerModel::Merge (const tgModel &m)
{
  tgModel::Merge (m);
...
void
TrackerModel::computeBoundingSphere ()
{
  m_boundingSphereRadius = 0.0;
...
// draws, prints
void
TrackerModel::Print () const
{
  tgModel::Print ();
  for (size_t i = 0; i < m_lines.size (); i++)
  {
    printf ("Edge %lu: %f %f %f,  %f %f %f\n", i, m_lines[i].start.x, m_lines[i].start.y, m_lines[i].start.z,
            m_lines[i].end.x, m_lines[i].end.y, m_lines[i].end.z);
  }
}

void
TrackerModel::drawCoordinates ()
{
...
vector<unsigned>
TrackerModel::getFaceUpdateList (const tgPose& pose, vec3 view, float minTexGrabAngle, bool use_num_pixels)
{
  int i, n;
...
TomGine::tgRect2Di
MakeBBPow2 (const TomGine::tgRect2Di &bb)
{
  tgRect2Di bb_new;
...
void
TrackerModel::getBoundingBox2D (TomGine::tgPose& pose, TomGine::tgRect2Di &bb, bool pow2)
{
...
void
TrackerModel::textureFromImage (TomGine::tgTexture2D &image, int width, int height, const tgPose& pose,
                                const std::vector<unsigned> &faceUpdateList, TomGine::tgCamera* m_cam)
{
  if (m_facepixellist.size () != m_faces.size ())
  {
    printf ("[TrackerModel::textureFromImage] Error, facepixellist out of date\n");
    return;
  }
  ...
  float y_max = 0.0;
  float x, y;
  for (size_t i = 0; i < faceUpdateList.size (); i++)
  {
    const unsigned &k = faceUpdateList[i];
    m_facepixellist[k] = 1;
    for (size_t j = 0; j < m_faces[k].v.size (); j++)
    {
  ...
  // store texture
  g_Resources->GetImageProcessor ()->copy (image, newpass->texture, (int)x_min, (int)y_min, unsigned (x_max - x_min),
  ...
  // clean up passes
  std::vector<unsigned> texturedfaces;
  std::vector<Pass*> new_passlist;
  for (size_t i = 0; i < m_passlist.size (); i++)
  { // parse through passlist
    Pass* p = m_passlist[i]; // current pass
    bool destroy = true;

    for (size_t j = 0; j < p->f.size (); j++)
    {
      if (find (texturedfaces.begin (), texturedfaces.end (), p->f[j]) == texturedfaces.end ())
      {
    ...
    if (destroy)
      ...
      new_passlist.push_back (m_passlist[i]);
  }
  m_passlist = new_passlist;
...
void
TrackerModel::useTexCoords (bool useTC)
{
...
void
TrackerModel::unwarpTexturesBox_hacky (const char* name)
{
...
bool
TrackerModel::isRedundant (TomGine::tgLine* e1)
{
  tgLine* e2;
  vec3 vs, ve;
...
void
TrackerModel::UpdateDisplayLists ()
{
...
void
TrackerModel::drawTexturedFaces ()
{ // draw only textured faces
  ...
  for (p = 0; p < (int)m_passlist.size (); p++)
  {
    ...
    for (i = 0; i < (int)m_passlist[p]->f.size (); i++)
    {
      ...
      //      if (m_passlist[p]->f[i] < 0 || m_passlist[p]->f[i] > m_faces.size ())
      //        printf ("[drawTexturedFaces ()] Error out of bounds\n");
...
void
TrackerModel::drawUntexturedFaces ()
{ // draw only untextured faces
  int i, j;
  tgFace* f;
...
void
TrackerModel::drawPass (bool colorful)
{ // draw faces using passlist and shader for texturing (modelviewmatrix)
  int p, i, j;
  tgFace* f;
  vec2 texCoords;
  ...
  for (i = 0; i < (int)m_faces.size (); i++)
  {
    ...
    if (m_facepixellist[i] == 0)
    {
      ...
      //      {
      //        TomGine::tgVertex &v = m_vertices[j];
      //        float s = 0.1f;
...
void
TrackerModel::drawFaces (bool colorful)
{ // draw all faces of model
  int i, j;
  tgFace* f;
...
void
TrackerModel::drawEdges ()
{ // draw all edges of model
  mat4 mv;
  mat3 rot;
  vec3 v_cam_object;
  float s = -0.001f; // = 1mm
  ...
  for (int i = 0; i < (int)m_lines.size (); i++)
  {
    glNormal3f (m_lines[i].end.x - m_lines[i].start.x, m_lines[i].end.y - m_lines[i].start.y,
...
void
TrackerModel::drawNormals (float normal_length)
{ // draw normals
  unsigned i, j;
  tgFace* f;
