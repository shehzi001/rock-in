...
private:
  ...
  tgShader* m_shadeGradientsX; ///< Fragment shader for computing gradients
  tgShader* m_shadeGradientsY; ///< Fragment shader for computing gradients
  ...
  tgShader* m_shadeSobelAdaptive; ///< Fragment shader for edge detection with sobel algorithm (with adaptive threshold)
  ...
  tgShader* m_shadeDotConvolution; ///< Fragment shader for dot product of rgb-(xyz)-components within a convolution cernel
...
public:
  ...
  void multiply(const tgTexture2D& src1, const tgTexture2D& src2, tgTexture2D& result, bool src1grad = false,
                bool src2grad = false);
  ...
  /** @brief Calculate total average luminance on current framebuffer */
  static float averageLuminance(GLenum internal = GL_RGBA32F);

  /** @brief Calculate average luminance per channel on current framebuffer */
  static vec3 average();
  ...
  /** @brief Gradients as centered differences in 4-neighbourhood
   *  @param source Input color image.
   *  @param gradientsX Gradients in X-direction
   *  @param gradientsY Gradients in Y-direction  */
  void gradients(const tgTexture2D& source, tgTexture2D& gradientsX, tgTexture2D& gradientsY);
  ...
  /** @brief Sobel edge detection calculating the gradients of an color image (with adaptive threshold).
   *    @brief The gradient is scaled to the range [0..1] and can be transformed back with:
   *                    grad = (vec2(R,G) - vec2(0.5,0.5)) * 2
   *            where R and G are the red and green component of a gradient image pixel.
   *    @param source   Input color image.
   ...
   *    @param normalise        Normalize the gradients (RG values).
   *    @param binary   Does not return gradients but 1 (white) if the magnitude of a gradient exceeds the threshold, 0 (black) otherwise. */
  void sobelAdaptive(const tgTexture2D& source, tgTexture2D& result, float adaptive_threshold = 0.01,
                     float offset = 0.0f, bool normalise = false, bool binary = false);
  ...
  void dotConvolute(const tgTexture2D& source, tgTexture2D& result, std::vector<float> kernel, float threshold = 0.0);
